...
def train(train_images_path, train_images_label, val_images_path=None, val_images_label=None, classes=2, epochs=20, batch_size=32,
          lr=0.0001, weight='./pretrained/swin_tiny_patch4_window7_224.pth', freeze=False, device='cuda:0', val=True, init=True):
    ...
    for epoch in range(epochs):
        ...
        f = open('./res_dir/train_res/data'+'/swin_res.txt', 'a')
        ...
        if best_acc < train_acc:
            best_acc = train_acc
            best_epoch = epoch
            torch.save(model.state_dict(), './res_dir/weights/data'+'/best_swin_model.pth')
    f1 = open('./res_dir/best_res/best.txt', 'a')
    f1.write('swin ' + 'epoch ' + str(best_epoch) + '  ' + str(best_acc) + '\n')


def predict(images_path, images_label, num_class=2, batch_size=4):
    ...
    model_weight_path = './res_dir/weights/data'+'/best_swin_model.pth'
    model = swin_tiny_patch4_window7_224(num_classes=num_class).to(device)
    model.load_state_dict(torch.load(model_weight_path, map_location=device))
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    score = cal_m(model=model, data_loader=loader, device=device, alo='swin', num=num_class)
    pd.DataFrame(np.array(score)).to_csv('./res_dir/label/data'  + '/swin' + '.csv', header=None, index=None)
    return score
